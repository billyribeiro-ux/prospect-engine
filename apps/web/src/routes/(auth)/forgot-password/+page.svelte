...
<svelte:head>
	<title>{messages.app.auth.forgotTitle} · {messages.app.productName}</title>
</svelte:head>

<h1 class="auth-title">{messages.app.auth.forgotTitle}</h1>
<form class="auth-form" method="post" action="/api/v1/auth/forgot-password">
	<label class="auth-label">
		<span>{messages.app.auth.emailLabel}</span>
		<input class="auth-input" type="email" name="email" autocomplete="email" required />
	</label>
	<button class="auth-submit" type="submit">{messages.app.auth.submitForgot}</button>
</form>
<p class="auth-footer">
	<a class="auth-link" href="/login">{messages.app.auth.signInLink}</a>
	&nbsp;·&nbsp;
